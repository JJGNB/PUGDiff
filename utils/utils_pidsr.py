...
def visulize_aop_dop(aop, dop):
    ...
    img_clone = dop.copy()
    img_clone[dop>0.3] = 0.3
    img_clone[dop<0] = 0
    img_clone = img_clone/0.3
    dop_map=img_clone
    dop_map=(dop_map* 255).astype(np.uint8)
    aop_map=aop.astype(np.uint8)
...
class GradientLoss(nn.Module):
    ...
    def forward(self, generated_img, target_img):
        ...
        loss_x = self.alpha * F.l1_loss(gen_grad_x, tgt_grad_x,reduction="none") + self.beta * F.l1_loss(generated_img, target_img,reduction="none")
        loss_y = self.alpha * F.l1_loss(gen_grad_y, tgt_grad_y,reduction="none") + self.beta * F.l1_loss(generated_img, target_img,reduction="none")
        
        return loss_x + loss_y

def polar_loss(output, gt, device):
    l1loss = nn.L1Loss(reduction="none").to(device)
    ...
    img_loss = gradientloss(output, gt)
    grad_loss=gradientloss(output_S0, gt_S0)
    ...
    return img_loss,10*grad_loss + 10*stokes_loss  + 10*aop_loss+ polar_loss
