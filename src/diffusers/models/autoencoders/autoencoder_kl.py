...
from .vae import Decoder, DecoderOutput, DiagonalGaussianDistribution, Encoder, PolTimeAwareEncoder, TimeAwareEncoder
...
class NoisePredictor(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    @apply_forward_hook
    def encode(
        self, x: torch.Tensor, timestep: Union[int, torch.Tensor], return_dict: bool = True
    ) -> Union[AutoencoderKLOutput, Tuple[DiagonalGaussianDistribution]]:
        """
        Encode a batch of images into latents.
    ...
    def forward(
        ...
        if sample_posterior:
            ...
            return posterior
class PolNoisePredictor(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    r"""
    A noise predicted model from the encoder of AutoencoderKL.
    ...
    def __init__(
        ...
        # pass init params to Encoder
        self.encoder = PolTimeAwareEncoder(
            in_channels=in_channels,
            out_channels=latent_channels,
            down_block_types=down_block_types,
            block_out_channels=block_out_channels,
    ...
    def encode(
        ...
        if self.use_slicing and x.shape[0] > 1:
            ...
            h,h_base = self.encoder(x, timestep)

        if not self.double_z:
            return h

        posterior = DiagonalGaussianDistribution(h)
        posterior_base=DiagonalGaussianDistribution(h_base)
        if not return_dict:
            return (posterior,posterior_base)

        return AutoencoderKLOutput(latent_dist=posterior),AutoencoderKLOutput(latent_dist=posterior_base)
    ...
    def forward(
        ...
        if not self.double_z:
            h,h_base = self.encode(sample, timesteps)
            return h,h_base
        else:
            posterior,posterior_base = self.encode(sample, timesteps)
            posterior=posterior.latent_dist
            posterior_base=posterior_base.latent_dist

        if sample_posterior:
            return posterior.sample(),posterior_base.sample()
        else:
            return posterior,posterior_base
