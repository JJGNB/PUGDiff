...
class Encoder(nn.Module):
    ...
    def forward(self, sample: torch.Tensor) -> torch.Tensor:
        ...
        return sample
class PolTimeAwareEncoder(nn.Module):
    r"""
    The `PolTimeAwareEncoder` layer of a variational autoencoder that encodes its input into a latent representation.
    ...
    def __init__(
        ...
        conv_out_channels = 2 * out_channels if double_z else out_channels
        self.conv_out = nn.Conv2d(block_out_channels[-1], conv_out_channels, 3, padding=1)
        # self.convres_out=nn.Conv2d(block_out_channels[-1], conv_out_channels, 3, padding=1)
        self.gradient_checkpointing = False
    ...
    def forward(
        ...
        if self.training and self.gradient_checkpointing:
            ...
            if is_torch_version(">=", "1.11.0"):
                for down_block in self.down_blocks:
                    sample = torch.utils.checkpoint.checkpoint(
                        create_custom_forward(down_block), sample, emb, use_reentrant=False
                    )
                # middle
                sample = torch.utils.checkpoint.checkpoint(
                    create_custom_forward(self.mid_block), sample, emb, use_reentrant=False
                )
            else:
                for down_block in self.down_blocks:
                    sample = torch.utils.checkpoint.checkpoint(
        ...
        sample = self.conv_norm_out(sample)
        sample = self.conv_act(sample)
        sample_base = self.conv_out(sample)
        sample_res = self.convres_out(sample)
        sample_base_mean,sample_base_logvar=sample_base.chunk(2,dim=1)
        sample_base_mean=sample_base_mean.repeat(1,4,1,1)
        sample_base_logvar=sample_base_logvar.repeat(1,4,1,1)
        sample_full=torch.cat([sample_base_mean,sample_base_logvar],dim=1)
        sample=sample_full+sample_res
        return sample,sample_base
...
class DiagonalGaussianDistribution(object):
    ...
    def sample(self, generator: Optional[torch.Generator] = None) -> torch.Tensor:
        ...
        return x
    def mse(self, other: "DiagonalGaussianDistribution") -> torch.Tensor:
        mean_loss=F.mse_loss(self.mean, other.mean,reduction='none')
        var_loss=F.mse_loss(self.var, other.var,reduction='none')
        mean_loss=torch.mean(mean_loss, dim=list(range(1, mean_loss.ndim)))
        var_loss=torch.mean(var_loss, dim=list(range(1, var_loss.ndim)))
    ...
    def partial_kl(self, other: "DiagonalGaussianDistribution" = None) -> torch.Tensor:
        if self.deterministic:
            return torch.Tensor([0.0])
        else:
            if other is None:
