...
class StableDiffusionInvEnhancePipeline(
    ...
    def prepare_latents(
        ...
        # get latents
        # noise=util_pol.chunkdim2batch(noise)
    ...
    def __call__(
        ...
        image_downsample:PipelineImageInput = None,
        ...
        **kwargs,
        ...
        latent_timestep_np=timesteps[:1].repeat((batch_size * num_images_per_prompt)//4)
        # 6. Prepare latent variables
        if getattr(self, 'start_noise_predictor', None) is not None:
            with torch.amp.autocast('cuda'):
                noise,_ = self.start_noise_predictor(
                    image_downsample, latent_timestep_np, sample_posterior=True, center_input_sample=True,
                )
        else:
            noise = None
